...
class TestDistillBertBase:
    ...
    def test_base(self):
        ...
        response = client.run_model(run_inputs, sign=False)
        origin_pred = model(torch.tensor(run_inputs).unsqueeze(0)).logits.detach()

        diff = (torch.tensor([response]) - origin_pred).sum().abs()
        self.assertLess(diff, 0.001)  # difference is <0.1%


class TestDistillBertSW(TestDistillBertBase, unittest.TestCase):
    simulation = True


class TestDistillBertHW(TestDistillBertBase, unittest.TestCase):
    simulation = False
